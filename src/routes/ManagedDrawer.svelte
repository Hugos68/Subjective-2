	...
	{:else if $drawerStore.id==='connect'}
		...
		<div class="p-12 mx-auto w-[min(50rem,100%)]">
			...
			<form class="flex flex-col items-center gap-6" method="POST">
				...
				<button class="btn btn-ghost-secondary mt-5" formaction="?/login" type="submit">Login</button>
				<label>
					<span class="text-center">Providers:</span>
					<button class="btn btn-ringed-primary" formaction="?/login&provider=google">
					...
					<button class="btn btn-ringed-primary" formaction="?/login&provider=discord">
					...
					<button class="btn btn-ringed-primary" formaction="?/login&provider=github">
		...
		{:else if $storeTab==="register"}
			<form class="flex flex-col items-center gap-6" method="POST">
				...
				<label class="w-full" for="email">
					<span>Email:</span>
					<input type="text" id="email" name="email" minlength="4" required>
				</label>
				<label class="w-full" for="password">
					<span>Password:</span>
					<input type="password" id="password" name="password" minlength="6" required>
				</label>
				<label class="w-full" for="confirmPassword">
					<span>Confirm Password:</span>
					<input type="password" id="confirmPassword" name="confirmPassword" minlength="6" required>
				</label>
				<button class="btn btn-ghost-secondary mt-5" formaction="?/register" type="submit">Register</button>
			</form>
		{/if}
		</div>
